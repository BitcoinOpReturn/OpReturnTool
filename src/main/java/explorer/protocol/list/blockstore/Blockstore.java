package explorer.protocol.list.blockstore;
...
public class Blockstore implements IPlatformTestable{
	...
	public Boolean init(Block block, Transaction tx){
		...
		for(TransactionOutput output : listOutputs){
			try{
				...
				if(tempString.startsWith(Codes.RETURN) && (tempString.contains(Codes.BRACKET + Codes.BLOCKSTORE_CODE1) || 
						tempString.contains(Codes.BRACKET + Codes.BLOCKSTORE_CODE2) || tempString.contains(Codes.BRACKET + Codes.BLOCKSTORE_CODE3))){
