package explorer.protocol.list.bitpos;
...
public class BitPos implements IPlatformTestable{
	...
	public Boolean init(Block block, Transaction tx){
		...
		for(TransactionOutput output : listOutputs){
			try{
				...
				if(tempString.startsWith(Codes.RETURN) && tempString.contains(Codes.BRACKET + Codes.BITPOS_CODE1)){
