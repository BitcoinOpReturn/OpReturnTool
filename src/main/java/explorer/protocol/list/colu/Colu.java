package explorer.protocol.list.colu;
...
public class Colu implements IPlatformTestable{
	...
	public Boolean init(Block block, Transaction tx) {
		...
		for(TransactionOutput output : listOutputs){
			...
			try{
				...
				if(tempString.startsWith(Codes.RETURN) && tempString.contains(Codes.BRACKET + Codes.COLU_CODE1)){
