package explorer.protocol.list.remembr;
...
public class Remembr implements IPlatformTestable{
	...
	public Boolean init(Block block, Transaction tx){
		...
		for(TransactionOutput output : listOutputs){
			try{
				...
				if(tempString.startsWith(Codes.RETURN) && (tempString.contains(Codes.BRACKET + Codes.REMEMBR_CODE1) || tempString.contains(Codes.BRACKET + Codes.REMEMBR_CODE2))){
