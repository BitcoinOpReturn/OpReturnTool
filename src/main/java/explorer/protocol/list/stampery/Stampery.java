package explorer.protocol.list.stampery;
...
public class Stampery implements IPlatformTestable{
	...
	public Boolean init(Block block, Transaction tx){
		...
		for(TransactionOutput output : listOutputs){
			try{
				...
				if(tempString.startsWith(Codes.RETURN) && ( tempString.contains(Codes.BRACKET + Codes.STAMPERY_CODE1) || tempString.contains(Codes.BRACKET + Codes.STAMPERY_CODE2) ||
						tempString.contains(Codes.BRACKET + Codes.STAMPERY_CODE3) || tempString.contains(Codes.BRACKET + Codes.STAMPERY_CODE4) || tempString.contains(Codes.BRACKET + Codes.STAMPERY_CODE5))){
