package explorer.protocol.list.ascribe;
...
public class Ascribe implements IPlatformTestable{
	...
	public Boolean init(Block block, Transaction tx){
		...
		for(TransactionOutput output : listOutputs){
			try{
				...
				if(tempString.startsWith(Codes.RETURN) && tempString.contains(Codes.BRACKET + Codes.ASCRIBE_CODE1)){
