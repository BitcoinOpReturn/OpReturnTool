package explorer.protocol.list.stampd;
...
public class Stampd implements IPlatformTestable{
	...
	public Boolean init(Block block, Transaction tx){
		...
		for(TransactionOutput output : listOutputs){
			try{
				...
				if(tempString.startsWith(Codes.RETURN) && tempString.contains(Codes.BRACKET + Codes.STAMPD_CODE1)){
