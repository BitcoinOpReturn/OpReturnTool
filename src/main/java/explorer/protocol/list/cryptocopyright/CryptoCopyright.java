package explorer.protocol.list.cryptocopyright;
...
public class CryptoCopyright implements IPlatformTestable{
	...
	public Boolean init(Block block, Transaction tx){
		...
		for(TransactionOutput output : listOutputs){
			try{
				...
				if(tempString.startsWith(Codes.RETURN) && (tempString.contains(Codes.BRACKET + Codes.CRYPTOCOPYRIGHT_CODE1) || tempString.contains(Codes.BRACKET + Codes.CRYPTOCOPYRIGHT_CODE2))){
