package explorer.protocol.list.proofofexistence;
...
public class ProofOfExistence implements IPlatformTestable{
	...
	public Boolean init(Block block, Transaction tx){
		...
		for(TransactionOutput output : listOutputs){
			try{
				...
				if(tempString.startsWith(Codes.RETURN) && tempString.contains(Codes.BRACKET + Codes.PROOFOFEXISTENCE_CODE1)){
