package explorer.protocol.list.factom;
...
public class Factom implements IPlatformTestable{
	...
	public Boolean init(Block block, Transaction tx){
		...
		for(TransactionOutput output : listOutputs){
			try{
				...
				if(tempString.startsWith(Codes.RETURN) && (tempString.contains(Codes.BRACKET + Codes.FACTOM_CODE1) || tempString.contains(Codes.BRACKET + Codes.FACTOM_CODE2) 
						|| tempString.contains(Codes.BRACKET + Codes.FACTOM_CODE3) || tempString.contains(Codes.BRACKET + Codes.FACTOM_CODE4))){
